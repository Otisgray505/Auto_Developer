...
function startTunnel(port = 3000) {
    logger_1.logger.info(`Starting Cloudflare Quick Tunnel for localhost:${port}`);
    ...
    const cloudflared = (0, child_process_1.spawn)('npx', ['cloudflared', 'tunnel', '--url', `http://localhost:${port}`], { shell: true });
    let urlFound = false;
    ...
    process.once('SIGINT', () => {
        logger_1.logger.info('Shutting down tunnel...');
        cloudflared.kill();
        process.exit(0);
    });
}
